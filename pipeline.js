...
async function cleanCsvDuplicates(csvFilePath) {
    ...
    try {
        ...
        for (let i = 0; i < headers.length; i++) { const h = headers[i].toLowerCase().trim(); if (standard.has(h) || seenInd.has(h)) continue; seenInd.add(h); keep.push(i); keepH.push(headers[i]); }
        ...
        for (let i = 0; i < indices.length; i++) {
            ...
            if (runMode === "both" || runMode === "script") {
                ...
                if (scriptSuccess) {
                    ...
                    const status = { index: idx, name: indicatorName, csvStatus: runMode === "both" ? "OK" : "SKIP", scriptStatus: "FAIL", csvError: `Script extraction failed: ${scriptErr}` };
                    updateStatusCsv(status);
                    results.push({ idx, name: indicatorName, csvStatus: runMode === "both" ? "OK" : "SKIP", scriptStatus: "FAIL", csvTime, scriptTime: "0", reason: `Script extraction failed: ${scriptErr}` });
                }
            } else {
